...
- **Fixes Applied:**
  - **F2 - Race condition fix**: Added retry mechanism using `requestAnimationFrame` to wait for sections to mount
  - **F9 - Debouncing**: Added `activeSectionRef` to prevent rapid re-renders, process only most intersecting section
- **Complete Code:**
  ```typescript
  'use client'
...
1. **Run Sanity verification script BEFORE starting development:**
   ```bash
   npm run verify:sections
   ```
   - This script checks that all 8 required page documents exist in Sanity
   - If any documents are missing, create them in Sanity Studio before proceeding
   - The script will exit with error code 1 if documents are missing
